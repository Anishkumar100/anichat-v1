...
export const LeftSidebar = ({ setBg, bg }) => {
  ...
  const {
    ...
    createGrp, setCreateGrp, isSunMode,
  } = useAppContext();

  const [search, setSearch]       = useState("");
  const [menuOpen, setMenuOpen]   = useState(false); // replaces group-hover (broken on mobile)
  ...
  const toggleDot=isSunMode ? "bg-purple-600 text-white"                   : "bg-yellow-400 text-black";
  ...
  const handleSelectUser = (user) => {
    ...
    setUnseenMessages(p => { const n={...p}; delete n[user._id]; return n; });
  };
  const handleSelectGroup = (group) => {
    setSelectedGroup(group); setSelectedUser(null);
    setUnseenMessages(p => { const n={...p}; delete n[`group_${group._id}`]; return n; });
  };

  return (
    <div className="bg-[#8185B2]/10 h-full flex flex-col text-white overflow-hidden">
      {/* ── Fixed top bar (logo + controls) ─────────────────────────── */}
      <div className="flex-shrink-0 px-4 pt-4 pb-2">
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-3" ref={menuRef}>
            ...
            {/* Three-dot menu — tap/click toggles, works on mobile */}
            <div className="relative">
              <img
                src={assets.menu_icon}
                alt="menu"
                className="max-h-5 cursor-pointer select-none"
                onClick={() => setMenuOpen(p => !p)}
              />
              {menuOpen && (
                <div className="absolute top-full right-0 mt-2 z-50 w-36 py-2 rounded-md bg-[#282142] border border-gray-600 shadow-xl">
                  <p onClick={() => { setMenuOpen(false); navigate("/profile"); }}
                    className="cursor-pointer text-sm px-4 py-2 hover:bg-white/10">Edit Profile</p>
                  <hr className="border-gray-600 mx-2" />
                  <p onClick={() => { setMenuOpen(false); setCreateGrp(true); navigate("/new-group"); }}
                    className="cursor-pointer text-sm px-4 py-2 hover:bg-white/10">Create Group</p>
                  <hr className="border-gray-600 mx-2" />
                  <p onClick={() => { setMenuOpen(false); logout(); }}
                    className="cursor-pointer text-sm px-4 py-2 hover:bg-white/10 text-red-400">Logout</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {/* Users */}
        <p className="text-xs text-gray-400 mb-2 mt-3">Users</p>
        {filteredUsers.length === 0 && <p className="text-xs text-gray-500 pl-2">No users found</p>}
        {filteredUsers.map(user => {
          ...
          return (
            <div
              ...
              className={`relative flex items-center gap-2 p-2 pl-2 rounded cursor-pointer text-sm transition-colors duration-200 ${hoverBg} ${isActive ? activeBg : ""}`}
            >
              <div className="relative flex-shrink-0">
                <img src={user.profilePic || assets.avatar_icon} alt=""
                  className="w-9 h-9 rounded-full object-cover" />
                {isOnline && (
                  <span className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full bg-green-500 border-2 border-[#282142]" />
                )}
              </div>
              ...
              {unread > 0 && (
                <span className={`ml-auto flex-shrink-0 text-xs h-5 w-5 flex justify-center items-center rounded-full text-white ${badgeBg}`}>
                  {unread}
                </span>
              )}
            </div>
        ...
        {/* Groups */}
        <p className="text-xs text-gray-400 mb-2 mt-5">Groups</p>
        {filteredGroups.length === 0 && <p className="text-xs text-gray-500 pl-2">No groups yet</p>}
