...
export const ChatContainer = () => {
  ...
  // InputEmojiPortal removed — emoji is now in MediaPanel inline above the input bar
    ...
    <div className="flex-shrink-0 flex items-center gap-2 p-2 sm:p-3 border-t border-white/5">
      <div className="flex-1 flex flex-col items-start bg-gray-100/12 px-3 rounded-2xl min-w-0">
        ...
        <div className="flex w-full items-center gap-1">
          ...
          <input type="file" id="chat-file" accept="image/png,image/jpeg,image/gif,image/webp" hidden onChange={handleFileSelect || handleImageSelect} />
          <label htmlFor="chat-file" className="cursor-pointer flex-shrink-0 opacity-60 hover:opacity-100 transition-opacity">
  ...
  // ── DM view ────────────────────────────────────────────────────────
  const dmOnline = isUserOnline ? isUserOnline(selectedUser?._id) : onlineUsers.includes(selectedUser?._id);
      ...
      <div className="h-full flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 py-3 px-3 sm:px-4 border-b border-stone-500 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-xs ${dmOnline ? "text-green-400" : "text-gray-400"}`}>
...
};  // ── Inline media panel — rendered in the flex column above the input bar ──
  // Inline (not portal) means it's always in the right place regardless of keyboard state.
  const MediaPanel = () => {
    if (!mediaPanel) return null;
    const accent = isSunMode ? "#f97316,#dc2626" : "#7c3aed,#9333ea";
    ...
    return (
      <div data-media-panel style={{
        ...
        background:"rgba(14,11,34,0.97)", overflow:"hidden",
      }}>
        {/* Tab Bar */}
        <div style={{ display:"flex", borderBottom:"1px solid rgba(255,255,255,0.08)", flexShrink:0 }}>
          {[
            ...
            <button key={tab.key} onClick={() => setMediaPanel(tab.key)}
              style={{ flex:1, padding:"8px 4px", border:"none", cursor:"pointer",
                background:"transparent", fontFamily:"inherit", fontSize:"0.78rem",
                fontWeight: mediaPanel === tab.key ? 700 : 400,
                color: mediaPanel === tab.key ? "white" : "rgba(255,255,255,0.4)",
                borderBottom: mediaPanel === tab.key
                  ? `2px solid ${accentSolid}` : "2px solid transparent",
                ...
                {filteredGifs.length === 0 ? (
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
                    ...
                    <p style={{ fontSize:"0.7rem", lineHeight:1.5 }}>
                      Click <strong>＋ Add GIF</strong> to upload a GIF from your device.
                    </p>
                  </div>
                ) : (
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"5px" }}>
                    {filteredGifs.map((gif, i) => (
                      <div key={i} style={{ position:"relative", aspectRatio:"1", borderRadius:"8px",
                        ...
                        onMouseEnter={e => { const b=e.currentTarget.querySelector(".gif-del"); if(b) b.style.opacity="1"; }}
                        onMouseLeave={e => { const b=e.currentTarget.querySelector(".gif-del"); if(b) b.style.opacity="0"; }}>
                        <img src={gif.url} alt={gif.name}
                          onClick={() => { sendMedia(gif.url); setMediaPanel(null); }}
                          style={{ width:"100%", height:"100%", objectFit:"cover", cursor:"pointer", display:"block" }} />
                        <button className="gif-del" onClick={e => { e.stopPropagation(); deleteFromGallery(gif.url); }}
              ...
              <div style={{ display:"flex", gap:"4px", padding:"6px 8px", flexShrink:0,
                ...
                <button onClick={() => setStickerPack("custom")}
                  style={{ padding:"4px 10px", borderRadius:"20px", border:"1px solid",
                    cursor:"pointer", fontSize:"0.68rem", whiteSpace:"nowrap",
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                    <label style={{ display:"flex", alignItems:"center", gap:"8px", cursor:"pointer",
                      ...
                      <div>
                        ...
                        <p style={{ color:"rgba(255,255,255,0.3)", fontSize:"0.65rem", marginTop:"2px" }}>
                          PNG, JPG, GIF or WebP
                        </p>
                      </div>
                    </label>
                    {myStickers.length === 0 ? (
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"5px" }}>
                        {myStickers.map((s, i) => (
                          <div key={i} style={{ position:"relative", aspectRatio:"1", borderRadius:"8px",
                            overflow:"hidden", background:"rgba(255,255,255,0.05)" }}
                            onMouseEnter={e => { const b=e.currentTarget.querySelector(".s-del"); if(b) b.style.opacity="1"; }}
                            onMouseLeave={e => { const b=e.currentTarget.querySelector(".s-del"); if(b) b.style.opacity="0"; }}>
                            <img src={s.url} alt={s.name} onClick={() => { sendMedia(s.url); setMediaPanel(null); }}
                              style={{ width:"100%", height:"100%", objectFit:"contain",
                                cursor:"pointer", display:"block", padding:"4px" }} />
                            <button className="s-del" onClick={e => { e.stopPropagation(); deleteCustomSticker(s.url); }}
