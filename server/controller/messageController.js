...
  try {
    console.log("--> [PROD DEBUG] uploadGif endpoint hit!");
    
    const { gif } = req.body;
    if (!gif) {
      console.log("--> [PROD DEBUG] No file found in req.body. Did Express strip it out?");
      return res.json({ success: false, message: "No file provided." });
    }

    console.log(`--> [PROD DEBUG] Received Base64 string. Length: ${(gif.length / 1024 / 1024).toFixed(2)} MB`);
    console.log("--> [PROD DEBUG] Attempting Cloudinary upload...");

    const upload = await cloudinary.uploader.upload(gif, {
      resource_type: "auto",
    });
    
    console.log("--> [PROD DEBUG] Cloudinary success! URL:", upload.secure_url);
    res.json({ success: true, url: upload.secure_url });
  } catch (error) {
    console.error("--> [PROD DEBUG] CLOUDINARY OR UPLOAD ERROR:", error);
    res.json({ success: false, message: error.message });
  }
};